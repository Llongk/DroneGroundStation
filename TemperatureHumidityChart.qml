...
Item {
    ...
    function drawSeries(ctx, data, minValue, maxValue, color,
        ...
        for (var i = 0; i < data.length; ++i) {
            ...
            var y = bottom - (Number(data[i]) - minValue)
    ...
    Canvas {
        ...
        onPaint: {
            ...
            ctx.font = "11px Arial"
            ctx.fillStyle = "#ff9c63"; ctx.fillText("● 温度 ℃", 18, 43)
            ctx.fillStyle = "#54d5eb"; ctx.fillText("● 湿度 %", 95, 43)
            ...
            root.drawSeries(ctx, root.temperatureData, -50, 100, "#ff9c63",
                            left, right, top, bottom)
            root.drawSeries(ctx, root.humidityData, 0, 100, "#54d5eb",
                            left, right, top, bottom)
