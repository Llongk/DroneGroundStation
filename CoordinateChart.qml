...
Item {
    ...
    function drawSeries(ctx, data, range, color, left, right, top, bottom) {
        ...
        for (var i = 0; i < data.length; ++i) {
            var x = data.length === 1 ? (left + right) / 2 : left + i * (right - left) / (
                                            data.length - 1);
            var y = bottom - (Number(data[i]) - range.min) * (bottom - top) / (range.max
                                                                               - range.min);
            if (i === 0)
                ctx.moveTo(x, y);
            else
    ...
    Canvas {
        ...
        onPaint: {
            ...
            ctx.font = "10px Arial";
            ctx.fillStyle = "#58d7ed";
            ctx.fillText("纬 " + latRange.min.toFixed(5) + " ~ " + latRange.max.toFixed(5), left,
                         height - 20);
            ctx.fillStyle = "#ffb56b";
            ctx.fillText("经 " + lonRange.min.toFixed(5) + " ~ " + lonRange.max.toFixed(5), left,
                         height - 7);
