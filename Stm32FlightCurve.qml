    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            ComboBox {
                ...
                // 将采样对象转换为当前指标的数值序列。
                model: root.series.map(function (item) {
                    return item.name;
                })
            ...
            Canvas {
                ...
                onPaint: {
                    ...
                    for (var p = 0; p < points.length; ++p) {
                        var x = points.length === 1 ? left : left + plotWidth * p / (points.length
                                                                                     - 1);
                        var value = points[p][item.key];
                        var pointY = top + (maxValue - value) / (maxValue - minValue) * plotHeight;
                        if (p === 0)
                            ctx.moveTo(x, pointY);
                        else
                            ctx.lineTo(x, pointY);
        ...
        Text {
            ...
            color: "#d9e6eb"
            font.pixelSize: 13
            text: root.dataPoints.length > 0 ? "当前：" + root.dataPoints[root.dataPoints.length
                                                                       - 1][root.series[root.selectedIndex].key].toFixed(
                                                   1) + " " + root.series[root.selectedIndex].unit :
                                               "当前：--"
        }
    }
}
